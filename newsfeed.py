...
def viewNewsArticles(topicArray,count = 50):
    df = pd.read_csv('backendScripting/data/politics18.csv')
    newsfeed = []
    renderedCount = 0
        ...
        for topic in topicArray:
            if (newsMatchedforTopic == 0):
                ...
                if taggedMinisters == ['nan']:
                    taggedMinisters = []
                if topic in tagged:
                    newsfeed.append({'author' : row[1].decode('utf-8'),'content' : row[2].decode('utf-8'),'date' : row[3],'tags' : tagged,'title' : row[5].decode('utf-8'),'url' : row[6],'website' : row[7],'taggedMinisters' :taggedMinisters })
                    renderedCount += 1
...
def returnRank(topic):
    df =pd.read_csv('backendScripting/data/rankedList.csv')
